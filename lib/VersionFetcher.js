...
module.exports = (commander, shell, randomDirGenerator, packageHandler) => {
  ...
  return {
    ...
    copyVersion: (remotePath, localPath, name) => {
      try {
        const localFile = path.join(localPath, name);
        const remoteFile = path.join(remotePath, name);
        let currPackage = packageHandler.readPackageJson(localFile);
        let remotePackage = packageHandler.readPackageJson(remoteFile);
