...
module.exports = (commander, shell, randomDirGenerator, packageHandler, options) => {
  options = options || {};
  ...
  let createdDirs = [];
  const flagsSuffix = options.packQuietly ? '--quiet' : ''

  return {
    fetch: (name, version) => {
      const randomDir = generateDirectory();
      shell.pushd(randomDir);
      return commander.exec(`npm pack ${name}@${version} ${flagsSuffix}`)
    ...
    cloneAndPack: (cwd) => {
      ...
      shell.pushd(randomDir);
      return commander.exec(`npm pack ${cwd} ${flagsSuffix}`)
